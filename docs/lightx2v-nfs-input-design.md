...
2. **realpath 归一后仍在 `<root>/` 之下**(挡 `..` 穿越 / 软链逃逸)。
3. **两种布局都接受**:`inputs/` 子树(本次物化的上传),以及产物子树
   `<feature>-<model>/YYYY/MM/DD/<user_id>/…`(引用既有产物,见 §4.2)。二者都形如
   `…/<user_id>/<file>`,故规则 4 对两者一视同仁。
4. **realpath 解析后的父目录名 == 请求体 user_id** —— 跨租户读堵死。
   注意必须在 realpath **之后**判:否则 `inputs/<自己>/` 下一个指向他人目录的软链
   会因为原始路径段读起来是自己而放行。放宽规则 3 之后,**这是唯一的租户分隔手段**,不可再削弱。
...
root 之外;realpath 之后的 user_id 段绑定挡住跨租户读。真正的信任边界是:
...
### 4.2 产物可直接作为输入(零拷贝,决策 2)

§4 规则 3 最初写的是"只允许 `inputs/` 子树",理由是"堵掉拿他人结果做 i2i 反推"。这条后来被
**有意放宽**:现在 `inputs/` 与产物子树都接受。

**动机**:图生图 / 关键帧生视频 / 参考生视频里,调用方常把上一步拿到的产物 URL 原样传回来。
那份字节本就在这块共享 NFS 上,原先 new-api 必须先把它读出来、再复制一份写进 `inputs/` 才能引用,
对一个 30 MB 的视频就是一次全量读 + 一次全量写,而这段跑在扣费之前,延迟直接叠加到用户感知上。
放开之后 new-api 直接把产物的相对路径当 `input_ref` 下发,读写都省掉。

**为什么这不削弱隔离**:被放弃的那条理由——"拿**他人**结果做反推"——从来不是靠前缀挡住的,
而是靠规则 4 的 user_id 段绑定。两种布局都形如 `…/<user_id>/<file>`,产物路径的父目录名同样是
所属用户,规则 4 原样生效;规则 2 的 realpath 收敛、规则 5 的存在性也都不变。前缀限制真正提供的
只是"禁止引用**自己**的产物",而这恰恰是本次要的能力。

**生命周期安全**:被引用的产物不会在任务跑之前被清掉。`video_storage_janitor._protected_day_dirs`
从任务 params 的输入路径字段反推 day dir 加保护,推导是 `parent.parent`,与布局无关——产物路径一旦
作为 `image_path` 等写进新任务 params,它的 day dir 就随该任务(非终态期间)一并受保护。前提是
`_INPUT_PATH_PARAM_KEYS` 覆盖了全部引擎输入字段,新增字段时必须同步,否则被引用的产物会被 TTL /
水位驱逐,任务重派时输入已消失。

**发布顺序**:门面**先行**(放开是向后兼容的,老 new-api 不受影响),再上 new-api。反过来的话
new-api 下发产物 ref 会被门面 400。new-api 侧另有开关(`media_storage.nfs_zero_copy_input`,
默认关)兜住这个窗口,确认门面已放开后再打开。
...
- 外部伪造 input_ref 指向他人文件:相对路径 + realpath-under-root + realpath 后的 user_id 段绑定,
  三重挡住。放宽子树限制(§4.2)后,第三道是唯一的租户分隔,不可削弱。
- 指向**他人**输出当输入:同上,user_id 段绑定挡住。指向**自己**的输出是有意支持的(§4.2 零拷贝)。
