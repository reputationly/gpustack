...
# == container path). This is how a Breeze TTS instance gets the RW fast-disk NFS
# mount that holds the async task output files (save_result_path) and, for the
# clone path, the reference clips (spk_audio_path) — both absolute NFS paths
# injected by the facade. Voice design needs no reference audio at all, so a
# design-only deployment still needs this mount purely for the outputs. Read from os.environ so a model deployer cannot mount arbitrary host
...
# The engine image bakes in the HTTP server (server.py) at WORKDIR
# /opt/breeze-infer with PYTHONPATH set to the same, so ``uvicorn server:app``
# resolves the module. GPUStack only
# injects the bound host/port; the checkpoints dir is passed via BREEZE_MODEL_DIR
# (the model_path, bind-mounted host==container by base._get_configured_mounts).
_SERVER_APP = "server:app"


class BreezeTTSServer(InferenceServer):
    def start(self):
        try:
            self._start()
    ...
    def _start(self):
        logger.info(f"Starting Breeze TTS model instance: {self._model_instance.name}")

        deployment_metadata = self._get_deployment_metadata()

        env = self._get_configured_env()
        # server.py reads the checkpoints dir from BREEZE_MODEL_DIR. The model
        ...
        env.setdefault("BREEZE_MODEL_DIR", self._model_path)
        # CUDA-graph tier. "nocodec" graphs every stage except the codec:
        # measured on A100-40G it runs at RTF 0.657 with an 8.67 GiB peak,
        # against 0.494 / 26.93 GiB for the full "all" tier. The only extra
        # thing "all" buys is time-to-first-audio (120 ms -> 55 ms), which the
        # facade's submit/poll/read-a-file contract cannot use, and it pins the
        # codec runtime to a single lane. Override per model env if a deployment
        # ever serves the streaming endpoint directly.
        env.setdefault("BREEZE_FAST_TIER", "nocodec")
    ...
    def _create_workload(
        ...
        if not image:
            raise ValueError("Failed to get Breeze TTS backend image")
        ...
        logger.info(
            f"Creating Breeze TTS container workload: {deployment_metadata.name}"
        )
        logger.info(
            f"With image: {image}, "
        ...
        logger.info(
            f"Created Breeze TTS container workload: {deployment_metadata.name}"
        )
    ...
    def _get_extra_mounts(self) -> List[ContainerMount]:
        ...
        base._get_configured_mounts). This is how a Breeze TTS instance gets its
    ...
    def _build_command_args(
        ...
        # host/port; the checkpoints dir is passed via BREEZE_MODEL_DIR (env),
        ...
        if user_backend_parameters:
            logger.warning(
                "Breeze TTS ignores backend_parameters %s: the serving command is "
                "uvicorn (no engine CLI); configure the engine via model env "
                "(BREEZE_* variables) instead.",
