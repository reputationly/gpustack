...
# == container path). This is how a YuE2 instance gets the RW fast-disk NFS mount
# that holds the async task outputs (save_result_path, plus the .abc/.json
# sidecars the engine writes next to each song) — absolute NFS paths injected by
...
# identical rationale in worker/backends/lightx2v.py.
_EXTRA_MOUNTS_ENV_KEY = "GPUSTACK_EXTRA_MOUNTS"

# The engine image bakes in the HTTP server (server.py) at WORKDIR /opt/yue2
# with PYTHONPATH set to the same, so ``uvicorn server:app`` resolves the
# module. GPUStack only injects the bound host/port; the weights dir is passed
# via YUE2_MODEL_DIR (the model_path, bind-mounted host==container by
# base._get_configured_mounts). That dir is YuE2's pipeline layout —
# pipeline.json + YuE2-3B/ + YuE2-Vae/ — so one model path carries both the
# backbone and the decoder.
_SERVER_APP = "server:app"


class YuE2Server(InferenceServer):
    def start(self):
        try:
            self._start()
    ...
    def _start(self):
        logger.info(f"Starting YuE2 model instance: {self._model_instance.name}")

        deployment_metadata = self._get_deployment_metadata()

        env = self._get_configured_env()
        # server.py reads the pipeline root from YUE2_MODEL_DIR. The model path is
        # bind-mounted host==container, so the same absolute path is valid inside
        # the container. Keep the engine air-gapped: never reach out to
        # HF/ModelScope at inference time.
        env.setdefault("YUE2_MODEL_DIR", self._model_path)
    ...
    def _create_workload(
        ...
        if not image:
            raise ValueError("Failed to get YuE2 backend image")
        ...
        logger.info(f"Creating YuE2 container workload: {deployment_metadata.name}")
        logger.info(
            f"With image: {image}, "
        ...
        logger.info(f"Created YuE2 container workload: {deployment_metadata.name}")
    ...
    def _get_extra_mounts(self) -> List[ContainerMount]:
        ...
        base._get_configured_mounts). This is how a YuE2 instance gets its RW
        fast-disk NFS mount holding the async task outputs (e.g.
    ...
    def _build_command_args(
        ...
        # host/port; the weights dir is passed via YUE2_MODEL_DIR (env), not a
        ...
        # The command target is uvicorn itself: an engine-style flag would abort
        # startup, and a user --host/--port would displace the scheduler-assigned
        # binding. The engine has no CLI surface (all its knobs are YUE2_* env
        # vars), so ignore user backend parameters entirely and say so in the log.
        user_backend_parameters = self._flatten_backend_param()
        if user_backend_parameters:
            logger.warning(
                "YuE2 ignores backend_parameters %s: the serving command is uvicorn "
                "(no engine CLI); configure the engine via model env (YUE2_* "
                "variables) instead.",
